<div align="center">
  
# 👋 Welcome traveler

**Electrical & Computer Science Student @ ENSEA**

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/kylian-triton)
[![Portfolio](https://img.shields.io/badge/Portfolio-255E63?style=for-the-badge&logo=About.me&logoColor=white)](https://YOUR_WEBSITE.com)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:YOUR_EMAIL@example.com)

</div>

---

## 🛠️ Tech Stack & Tools

* **Languages:** ![C](https://img.shields.io/badge/-C-00599C?logo=c&logoColor=white) ![Python](https://img.shields.io/badge/-Python-3776AB?logo=python&logoColor=white) ![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?logo=javascript&logoColor=black) ![SQL](https://img.shields.io/badge/-SQL-336791?logo=postgresql&logoColor=white)
* **Hardware & Electronics:** ![KiCad](https://img.shields.io/badge/-KiCad-314CB6?logo=kicad&logoColor=white) ![MATLAB](https://img.shields.io/badge/-MATLAB-e16737?logo=mathworks&logoColor=white) ![Lab Equipment](https://img.shields.io/badge/-Oscilloscopes_&_GBF-4B0082?logo=soundcharts&logoColor=white)
* **Software & Web:** ![TensorFlow](https://img.shields.io/badge/-TensorFlow-FF6F00?logo=tensorflow&logoColor=white)
* **Tools & Environment:** ![Git](https://img.shields.io/badge/-Git-F05032?logo=git&logoColor=white) ![Linux](https://img.shields.io/badge/-Linux-FCC624?logo=linux&logoColor=black) ![LaTeX](https://img.shields.io/badge/-LaTeX-008080?logo=latex&logoColor=white)

---

## 💼 Experience

### 🔬 Electronics & Signal Processing Intern
**[INPHYNI](https://inphyni.univ-cotedazur.fr/) & [LEAT](https://leat.univ-cotedazur.fr/) (CNRS/UniCA)** | *Stage 1A*
* **Context:** Worked on a laser interferometry setup designed to be interpreted by an embedded Artificial Intelligence.
* **My Impact:** Tracked and eliminated electromagnetic interference (EMI) and ground loops to provide a clean, high-fidelity signal to the AI. Characterized high-speed amplifiers and designed a custom stabilized power supply along with an analog active filtering PCB using **KiCad**.
* **Tech Stack:** KiCad, Python (Instrument Automation / Bode Plots), Electronics Troubleshooting.

---

## 🚀 Featured Projects

### 1. [Stroboscopic Lamp (1st Year Project)](#)
* **What it does:** A custom-built stroboscopic lamp designed to freeze high-speed motion using precisely timed light flashes.
* **Tech Stack:** PCB Design, Routing, Electronics, Project Management.
* **My Impact:** Led the hardware development by designing the circuit, routing the PCB, and managing task assignments and workflow across the project team.

### 2. [OpenAstroTracker (Upcoming 2nd Year Project)](https://wiki.openastrotech.com/OpenAstroTracker)
* **What it does:** An open-source, 3D-printed equatorial tracking mount for DSLR cameras to enable long-exposure astrophotography.
* **Tech Stack:** 3D Printing, Microcontrollers (Arduino/ESP32), Stepper Motors, C++.
* **My Impact:** *(Work in progress)* I will be building and assembling the electromechanical system, wiring the motor drivers, and configuring the software for precise celestial tracking.

